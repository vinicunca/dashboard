...
export class Memory<T = any, V = any> {
  ...
  //   if (!isUnset(time) && time < new Date().getTime()) {
  //     this.remove(key);
  //   }
  ...
  resetCache(cache: { [K in keyof T]: Cache }) {
    Object.keys(cache).forEach((key) => {
      ...
      if (item && item.time) {
        ...
        if (expires > now) {
          this.set({ key: k, value: item.value, expires });
        }
      }
    });
  }
  ...
  clear() {
    objectKeys(this.cache).forEach((key) => {
